...
namespace
{
    void LoadOpenSSLProviders()
    {
        static OSSL_PROVIDER* defaultProvider =
            OSSL_PROVIDER_load(nullptr, "default");

        static OSSL_PROVIDER* legacyProvider =
            OSSL_PROVIDER_load(nullptr, "legacy");

        ASSERT(defaultProvider != nullptr);
        ASSERT(legacyProvider != nullptr);
    }
}
#endif
...
Trinity::Crypto::ARC4::ARC4() : _ctx(EVP_CIPHER_CTX_new())
{
    ...
    _cipher = EVP_CIPHER_fetch(nullptr, "RC4", "provider=legacy");
...
void Trinity::Crypto::ARC4::Init(uint8 const* seed, size_t len)
{
    ...
    int result2 = EVP_EncryptInit_ex(
        _ctx,
        nullptr,
        nullptr,
        seed,
...
void Trinity::Crypto::ARC4::UpdateData(uint8* data, size_t len)
{
    ...
    int result2 = EVP_EncryptFinal_ex(
        _ctx,
        data,
        &outlen
    );
